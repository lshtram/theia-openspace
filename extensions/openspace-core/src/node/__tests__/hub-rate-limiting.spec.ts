...
describe('RateLimiter (Issue 7)', () => {
    ...
    it('cleanup() removes stale entries older than 60 seconds', () => {
        ...
        expect((limiter as unknown as { counters: Map<string, { count: number; windowStart: number }> }).counters.size, 'stale entries should be removed').to.equal(0);
    });
    ...
    it('cleanup() preserves entries that are still within the 60-second window', () => {
        ...
        expect((limiter as unknown as { counters: Map<string, { count: number; windowStart: number }> }).counters.size, 'recent entry should be preserved').to.equal(1);
    });
